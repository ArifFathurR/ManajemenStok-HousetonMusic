...
export default function GeneralLayout({ header, children }) {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-24 md:pb-0">
            {/* Desktop Navbar */}
            <nav className="hidden md:block pt-6 pb-2 px-4 md:px-6">
                <div className="mx-auto max-w-7xl">
                    <div className="flex items-center justify-between gap-4">
                        <div className="bg-white rounded-full px-6 py-3 shadow-sm flex items-center min-w-fit">
                            <img src="/images/logo-houston.png" alt="Houston Music" className="h-8 w-auto object-contain" />
                        </div>

                        <div className="bg-white rounded-full p-1.5 shadow-sm flex items-center overflow-x-auto max-w-[400px] lg:max-w-none scrollbar-hide">
                            {navItems.map((item) => (
                        ...
                        <div className="bg-white rounded-full pl-2 pr-2 py-2 shadow-sm flex items-center gap-2">
                            <div className="relative group hidden lg:block">
                                ...
                                <input type="text" placeholder="Search..." className="block w-48 xl:w-64 rounded-full border-0 bg-gray-50 py-2.5 pl-10 pr-4 text-sm text-gray-900 focus:ring-2 focus:ring-indigo-500 focus:bg-white" />
                            </div>
            ...
            {/* Mobile Top Bar */}
            <div className="md:hidden bg-white border-b border-gray-200 shadow-sm sticky top-0 z-40 px-4 py-3 flex justify-between items-center">
                <img src="/images/logo-houston.png" alt="Houston Music" className="h-8 w-auto object-contain" />
                <img src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=random`} alt={user.name} className="h-8 w-8 rounded-full" />
            </div>
            ...
            {/* Mobile Bottom Nav */}
            <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 shadow z-50 h-16 flex justify-around items-center px-2">
                {mobileNavItems.map((item) => {
                    const Icon = item.active ? item.iconSolid : item.icon;
                    return (
                        <Link key={item.name} href={item.href} className={`flex flex-col items-center justify-center w-full h-full ${item.active ? 'text-gray-900 font-bold' : 'text-gray-400 hover:text-gray-600'}`}>
