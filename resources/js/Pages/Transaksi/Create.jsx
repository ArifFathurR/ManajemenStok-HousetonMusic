...
export default function Create({ auth, products }) {
    ...
    // --- 5. KALKULASI & SYNC DATA (FIX BUG DISINI) ---
    ...
    // FIX: Pastikan customerMoney selalu update mengikuti Grand Total jika metode bukan Cash
    useEffect(() => {
                ...
                <div className="flex-grow flex flex-col h-full overflow-hidden border-r border-slate-100 print:hidden">
                    <header className="bg-white p-4 flex flex-col sm:flex-row gap-4 justify-between items-center z-20 border-b border-slate-100">
                        ...
                        <div className="relative group w-full sm:w-64">
                            ...
                            <input type="text" placeholder="Cari produk atau kategori..." className="w-full bg-slate-50 border-none rounded-xl py-2 pl-10 pr-4 text-sm focus:ring-2 focus:ring-black transition-all shadow-inner" value={search} onChange={(e) => setSearch(e.target.value)} />
                        </div>
                    </header>
                                        ...
                                        <div key={product.id} onClick={() => handleProductClick(product)} className={`bg-white p-4 rounded-[2rem] border transition-all duration-300 cursor-pointer group flex flex-col active:scale-95 shadow-sm ${product.stok <= 0 && !product.is_variant ? 'border-red-200 opacity-50' : 'border-slate-100 hover:border-black'}`}>
                                            <div className="aspect-square bg-slate-50 rounded-[1.5rem] mb-4 overflow-hidden flex items-center justify-center text-slate-200 group-hover:bg-slate-100 transition-colors relative">
                                                {product.gambar_utama && !hasError ? (
                                                    <img src={`/storage/${product.gambar_utama}`} alt={product.nama_produk} className="w-full h-full object-contain p-2 mix-blend-multiply group-hover:scale-110 transition-transform duration-500" onError={() => handleImageError(product.id)} loading="lazy" />
                                                ) : (
                                                    <div className="flex flex-col items-center justify-center text-slate-300"><FiBox size={32} className="opacity-20" /></div>
                                                )}
                                                {product.stok <= 0 && !product.is_variant && (
                                                    <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center"><span className="bg-red-500 text-white text-[8px] font-black px-2 py-1 rounded-full shadow-lg">HABIS</span></div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex justify-between items-end mt-auto">
                                                <span className="font-[900] text-sm text-slate-900 leading-none">Rp {(channel === 'online' ? product.harga_online : product.harga_offline).toLocaleString()}</span>
                ...
                <div className={`fixed lg:static inset-x-0 bottom-0 z-40 bg-white border-l border-slate-100 flex flex-col transition-all mb-11 duration-700 ease-in-out print:hidden ${isCartExpanded ? 'h-[85vh]' : 'h-[80px] lg:h-full lg:w-[400px] xl:w-[450px]'}`}>
                    ...
                    <div className="p-4 md:p-6 border-b border-slate-50 flex justify-between items-center cursor-pointer lg:cursor-default shrink-0" onClick={() => window.innerWidth < 1024 && setIsCartExpanded(!isCartExpanded)}>
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={(e) => { e.stopPropagation(); if (cart.length > 0 && confirm('Hapus semua item dari keranjang?')) setCart([]); }} className="lg:flex hidden text-red-500 hover:bg-red-50 p-2 rounded-xl active:scale-90"><FiTrash2 size={18} /></button>
                            ...
                            cart.map(item => (
                                <div key={item.cartItemId} className="flex justify-between items-center bg-slate-50/50 p-4 rounded-[1.5rem] border border-slate-100 group">
                                    <div className="max-w-[60%]">
                                        ...
                                        <p className="text-[10px] font-bold text-slate-400 mt-1 italic uppercase tracking-tighter">@ Rp {(channel === 'online' ? item.harga_online : item.harga_offline).toLocaleString()}</p>
                                    </div>
                    ...
                    <div className={`p-6 md:p-8 bg-slate-50/80 backdrop-blur-lg border-t border-slate-200 lg:rounded-none rounded-t-[2.5rem] space-y-4 shadow-inner ${!isCartExpanded && 'hidden lg:block'}`}>
                        <div className="grid grid-cols-2 gap-3">
                            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm focus-within:border-black transition-colors">
                                ...
                                <input type="number" className="w-full bg-transparent border-none p-0 font-black text-sm focus:ring-0" value={discount || ''} onChange={(e) => setDiscount(Number(e.target.value) || 0)} placeholder="0" />
                            </div>
                            <div className={`bg-white p-3 rounded-xl border border-slate-200 shadow-sm transition-colors ${paymentMethod !== 'cash' ? 'opacity-100 bg-slate-50' : 'focus-within:border-blue-600'}`}>
                                ...
                                {/* FIX CSS: Added disabled styling to look active but locked */}
                                <input 
                                    type="number" 
                                    className="w-full bg-transparent border-none p-0 font-black text-sm focus:ring-0 text-blue-600 disabled:text-blue-600 disabled:opacity-100 disabled:cursor-not-allowed" 
                                    value={customerMoney || ''} 
                        ...
                        {/* FIX: Use hidden class instead of unmounting to keep layout stable if needed, or stick to conditional rendering but ensure container handles it */}
                        <div className={`flex gap-2 overflow-x-auto no-scrollbar py-1 transition-all duration-300 ${paymentMethod === 'cash' ? 'opacity-100 max-h-12' : 'opacity-0 max-h-0 overflow-hidden'}`}>
                            {quickAmounts.map(amount => (
                                <button key={amount} onClick={() => setCustomerMoney(amount)} className="whitespace-nowrap px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-[10px] font-black hover:bg-black hover:text-white transition-all shadow-sm">
                                    +{(amount / 1000).toFixed(0)}K
                                </button>
                            ))}
                            <button onClick={() => setCustomerMoney(grandTotal)} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-[10px] font-black shadow-lg shadow-blue-200 active:scale-95 transition-transform">PAS</button>
                        </div>
                        ...
                        <div className="pt-2 border-t border-slate-200 space-y-1">
                            ...
                            <div className="flex justify-between items-center font-black text-lg italic tracking-tighter uppercase text-slate-400 pt-2 border-t border-slate-100">
                                ...
                                <span className="text-black text-3xl font-[900] tracking-tighter italic leading-none">Rp {grandTotal.toLocaleString()}</span>
                            </div>
                            
                            {/* Logic Kembalian: Tetap render div kosong jika bukan cash agar layout tidak lompat (opsional, saat ini logic disembunyikan) */}
                            {paymentMethod === 'cash' && customerMoney > 0 && (
                                <div className="flex justify-between items-center text-[10px] font-black uppercase italic pt-2 animate-in slide-in-from-top-1 fade-in duration-300">
                                    <span className={change >= 0 ? 'text-green-600' : 'text-red-500'}>Kembalian</span>
                                    <span className={change >= 0 ? 'text-green-600 font-[900]' : 'text-red-500 font-bold'}>Rp {change.toLocaleString()}</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2 py-2">
                            {[{ id: 'cash', label: 'CASH', icon: FiDollarSign }, { id: 'debit', label: 'DEBIT', icon: FiCreditCard }, { id: 'qr', label: 'QRIS', icon: FiSmartphone }].map(method => (
                                <button key={method.id} onClick={() => setPaymentMethod(method.id)} className={`flex flex-col items-center py-3 rounded-[1.2rem] border-2 transition-all duration-300 ${paymentMethod === method.id ? 'bg-black text-white border-black shadow-2xl scale-105' : 'bg-white border-slate-100 text-slate-300 hover:border-slate-200'}`}>
                                    <method.icon size={18} />
                                    <span className="text-[8px] font-black tracking-widest uppercase mt-1.5">{method.label}</span>
                                </button>
                            ))}
                        </div>

                        <button disabled={cart.length === 0 || (paymentMethod === 'cash' && customerMoney < grandTotal) || isProcessing} onClick={handleFinalize} className={`w-full py-5 rounded-[2rem] font-black text-sm tracking-[0.1em] transition-all duration-500 shadow-sm ${cart.length === 0 || (paymentMethod === 'cash' && customerMoney < grandTotal) || isProcessing ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-black text-white hover:bg-slate-800 active:scale-95 shadow-black/20'}`}>
                            {isProcessing ? 'MEMPROSES...' : paymentMethod === 'cash' && customerMoney < grandTotal && cart.length > 0 ? 'UANG KURANG' : 'PROSES TRANSAKSI'}
                        </button>
                    </div>
                    ...
                    <div className={`lg:hidden flex justify-between items-center px-8 py-4 h-[80px] shrink-0 ${isCartExpanded && 'hidden'}`} onClick={() => setIsCartExpanded(true)}>
                        <h4 className="font-black text-xl tracking-tighter italic text-black leading-none">Rp {grandTotal.toLocaleString()}</h4>
                    ...
                    <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95 duration-300">
                        ...
                        <div className="grid grid-cols-1 gap-2">
                            {selectedProduct.variants.map(variant => (
                                <button key={variant.id} onClick={() => addToCart(selectedProduct, variant)} disabled={variant.stok <= 0} className={`flex items-center justify-between gap-4 p-4 rounded-2xl border-2 transition-all group text-left ${variant.stok <= 0 ? 'border-slate-100 opacity-50 cursor-not-allowed' : 'border-slate-50 hover:border-black active:scale-95'}`}>
                                    <div className="flex items-center gap-3">
                                        {variant.gambar && <div className="w-10 h-10 bg-slate-50 rounded-lg overflow-hidden shrink-0"><img src={`/storage/${variant.gambar}`} alt={variant.name} className="w-full h-full object-cover" /></div>}
                                        <span className="font-black text-xs uppercase tracking-tight group-hover:translate-x-1 transition-transform">{variant.name}</span>
                                    </div>
                    ...
                    <div className="print-area bg-white w-full max-w-sm rounded-[3rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 print:shadow-none print:w-full print:rounded-none">
                        <div className="p-8 space-y-6 text-slate-800">
                            <div className="text-center space-y-1">
                                <h2 className="font-[900] text-2xl italic tracking-tighter uppercase leading-none">{auth.user.name || 'Toko Saya'}</h2>
                                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.3em] italic">Struk Pembayaran</p>
                                ...
                                {cart.map(item => (
                                    <div key={item.cartItemId} className="text-xs font-bold leading-tight uppercase tracking-tight text-slate-700">
                                        <div className="flex justify-between mb-1">
                                            <span className="max-w-[70%]">{item.nama_produk} {item.variantName && `(${item.variantName})`}</span>
                                            <span>Rp {(item.qty * (channel === 'online' ? item.harga_online : item.harga_offline)).toLocaleString()}</span>
                                        </div>
                                        <p className="text-[9px] text-slate-400 italic lowercase">{item.qty} x Rp {(channel === 'online' ? item.harga_online : item.harga_offline).toLocaleString()}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-1 text-xs font-bold uppercase tracking-widest text-slate-500">
                                ...
                                {discount > 0 && (<div className="flex justify-between text-red-500 italic"><span>Diskon</span><span>- Rp {discount.toLocaleString()}</span></div>)}
                                <div className="flex justify-between font-[900] text-2xl pt-4 italic text-black leading-none border-t border-slate-100 tracking-tighter uppercase"><span>Total</span><span>Rp {grandTotal.toLocaleString()}</span></div>
                            </div>
                            <div className="bg-slate-50 p-4 rounded-[1.5rem] space-y-1 border border-slate-100">
                                ...
                                {paymentMethod === 'cash' && (
                                    <>
                                        <div className="flex justify-between text-[10px] font-black text-slate-400 border-t border-slate-200 pt-2 mt-1"><span>Tunai</span><span className="text-slate-900">Rp {customerMoney.toLocaleString()}</span></div>
                                        <div className="flex justify-between text-[10px] font-black border-t border-slate-200 pt-2 mt-1 font-[900] tracking-widest uppercase italic leading-none"><span className="text-green-600">Kembali</span><span className="text-green-600 text-sm font-sans tracking-normal">Rp {change.toLocaleString()}</span></div>
                                    </>
                                )}
                            </div>
